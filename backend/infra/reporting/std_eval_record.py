...
def build_record_pdf(record: dict[str, Any], out_pdf: str | Path) -> Path:
    ...
    rows: list[list[Any]] = [
        ...
        [
            ...
            P(ev[0].get("name", "—")),
            P("持证类型"),
            P(ev[0].get("cert_type", "—")),
            P("证书有效期"),
            P(ev[0].get("valid_until") or "未注明"),
        ],
        [
            P("检测方法"),
            P(film["kind"]),
            P("透照布置"),
            P(film["exposure_layout"]),
            P("焊缝形式/焊接方法"),
            P(f"{film['weld_form']} / {film['weld_method']}"),
        ],
        [
            P("标准测试集缺陷数量（个）"),
            P(film["n_defects"]),
            P("缺陷底片数量（张）"),
            P(film["n_defect_images"]),
            P("无缺陷底片数量（张）"),
            P(film["n_no_defect_images"]),
        ],
        [P("各类缺陷占比"), P(film["class_distribution"]), P(""), P(""), P(""), P("")],
        [
            P("标注人员"),
            P("；".join(x.get("name", "") for x in lb if isinstance(x, dict))),
            P("持证类型"),
            P("；".join(x.get("cert_type", "") for x in lb if isinstance(x, dict))),
            P("资质校验"),
            P("通过" if per["qualified"] else "不符合：" + "；".join(per["issues"][:3])),
        ],
        ...
        [
            P("严格口径（IOU≥0.3）"),
            P(
                f"KDR={met['kdr_strict']:.2%}；WDR={met['wdr_strict']:.2%}；TDR={met['tdr_strict']:.2%}；FRR={met['frr_strict']:.2%}"
            ),
            P(""),
            P(""),
            P(""),
        ...
        [
            ...
            P(f"{met['frr']:.2%}"),
            P("标准分级"),
            P(grad["level_standard"] or "未定级"),
            P("严格口径分级"),
            P(grad["level_strict"] or "未定级"),
        ],
        [
            P("系统分级结果"),
            P(
                f"{_chk(level, 'L1')} L1　{_chk(level, 'L2')} L2　{_chk(level, 'L3')} L3　{_chk(level, 'L4')} L4　{('（记录值，取较差口径）' if level else '（未定级）')}"
            ),
        ],
        [
            P("风险分析结果"),

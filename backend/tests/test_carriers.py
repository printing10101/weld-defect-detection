...
def test_register_requires_secadmin():
    ...
    with principal_role("sysadmin"), TestClient(app) as c:
        resp = c.post(
            "/api/v1/carriers",
...
def test_borrow_return_flow():
    ...
    with principal_role("secadmin"), TestClient(app) as c:
        _register(c, carrier_id)
    with principal_role("sysadmin", username="操作员A"), TestClient(app) as c:
        resp = c.post(f"/api/v1/carriers/{carrier_id}/borrow", json={"note": "外借评片"})
        assert resp.status_code == 200
        assert resp.json()["status"] == "borrowed"
        # 重复借用 → 409
        assert c.post(f"/api/v1/carriers/{carrier_id}/borrow").status_code == 409
...
def test_destroy_self_confirm_rejected():
    """销毁双确认：同账号自确认 409 拒绝。"""
    carrier_id = _new_carrier_id()
    with principal_role("secadmin", username="保密员甲"), TestClient(app) as c:
        _register(c, carrier_id)
        req = c.post(
            f"/api/v1/carriers/{carrier_id}/destroy-request",
    ...
    with principal_role("sysadmin", username="保密员甲"), TestClient(app) as c:
        resp = c.post(f"/api/v1/carriers/{carrier_id}/destroy-confirm")

...
| 模型激活/评估 | 是 | — | — |
| 密级设定/变更（C-10） | — | 是 | — |
| 载体登记/销毁发起（C-12） | —（仅销毁确认） | 是（销毁发起） | — |
| 导出审批（C-14） | — | 是 | — |
| 告警处置/账号解锁 | 仅解锁 | 是 | — |
| 主审计链读取 | 是 | 是 | 是（只读） |
| **独立安全审计链**读取（C-19） | — | — | 是（只读） |
| 评片/复核/检索等业务 | 是（登录即可） | 是 | 是 |
...
边界说明：主机名目的地址需先 DNS 解析才能判定，解析本身可能产生 DNS 查询
外发——离线部署的同步端点应配置 **IP 字面量**；本防护只覆盖本软件进程，
不能替代 OS 层出站防火墙（两者互补，见第四节第 4 步）。

## 六、IPC 一次性启动令牌（C-17）

前端（桌面壳 WebView）与本地后端的进程间通信加固：
...
- 桌面壳（Electron）在端口就绪后读取令牌文件并注入 WebView
  （`window.__IPC_TOKEN__`），前端 `services/api.ts` 统一携带该头。

威胁模型：本机前后端为回环明文 HTTP，令牌**不解决传输加密**——

...
> 注：前端类型以手写 `src/src/types/api.ts` 为唯一真相（镜像后端 Pydantic 响应模型），
> 不依赖 openapi 自动生成产物，由 `backend/tests/test_frontend_contract.py` 做字段对账防漂移。
...
* 标准数值未授权前 `StandardGrader.grade` 必须熔断（need\_review=True，不输出级别）。

## 尚未完成（按优先级，均需人工/数据，非代码能解）

1. **真实标注数据**：真实稀有类仅 28 张人工标注，其余为 AI 预标注；
   需补足 \~100 张真实标注（尤其真实裂纹）才能把检测从"召回优先预筛"
   提升到"可定级"。
